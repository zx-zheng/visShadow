...
public class PoissonDiskSampler implements Sampler<Vector2DDouble>
{
	...
	public List<Vector2DDouble> sample()
	{
		...
		List<Vector3DDouble> grid[][] = new List[gridWidth][gridHeight];

		for (int i = 0; i < gridWidth; i++)
		{
			for (int j = 0; j < gridHeight; j++)
			{
				grid[i][j] = new LinkedList<Vector3DDouble>();
			}
		}
	...
	private boolean addNextPoint(List<Vector3DDouble>[][] grid, List<Vector2DDouble> activeList,
			List<Vector2DDouble> pointList, Vector2DDouble point)
	{
		boolean found = false;
		double fraction = distribution.getDouble(point.x,  point.y);
		...
		if ((q.x >= p0.x) && (q.x < p1.x) && (q.y > p0.y) && (q.y < p1.y))
		{
			...
			for (int i = Math.max(0, qIndex.x - 2); (i < Math.min(gridWidth, qIndex.x + 3)) && !tooClose; i++)
			{
				...
				{
					for (Vector3DDouble gridPoint : grid[i][j])
					{
						if (Vector3DDouble.distancexy(gridPoint, q) < minDist * fraction
						    || Vector3DDouble.distancexy(gridPoint, q) < gridPoint.z)
						{
							tooClose = true;
						}
					}
	...
	private void addFirstPoint(List<Vector3DDouble>[][] grid, List<Vector2DDouble> activeList,
